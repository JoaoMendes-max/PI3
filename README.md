...
On microcontrollers (MCUs) there is no MMU, so there is no virtual memory or address translation. All physical memory is shared by software. This makes it easy for tasks to access memory belonging to other tasks, leading to security vulnerabilities and unpredictable system behavior.
...
3. FreeRTOS configuration for U-Mode: the original RISC-V port runs in M-Mode; MEMGUARD provides a configuration and modifications so that tasks run in U-Mode and the kernel remains protected. This requires careful handling of exceptions and privilege level transitions.

## Memory Protection Comparison

| Feature | Baseline | M-Mode Isolated | U-Mode Isolated |
|---------|----------|-----------------|-----------------|
| Kernel mode | M-Mode | M-Mode | M-Mode |
| Tasks mode | M-Mode | M-Mode | U-Mode |
| PMP context switch | No reconfiguration | No reconfiguration | Reconfiguration per task |
| Isolation between tasks | None | Yes | Yes |
| Isolation granularity | - | - | For physical memory regions |
| Response to critical fault | Silence (and continue) | Store Access Fault, system panic | Store Access Fault, system panic |
